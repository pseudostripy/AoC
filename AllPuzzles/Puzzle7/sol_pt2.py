...
CARDCONVERT = {"A": 14, "K": 13, "Q": 12, "T": 10, "J": 1}
...
def get_hand_type(hnd):
    hnd_noJ = [h for h in hnd if h != 1]  # 1 represents J
    if hnd_noJ == []:
        return HT.FIVEKIND  # nice meme.
    htots = sorted(
        list(filter(lambda x: x != 0, np.bincount(hnd_noJ))), reverse=True)
    nj = len(hnd) - len(hnd_noJ)  # number of jokers
    htots[0] += nj  # best play to max htype
    return get_full_hand(htots)


def get_full_hand(htots):
    f = htots[0]
    if f == 5:
        return HT.FIVEKIND
    if f == 4:
